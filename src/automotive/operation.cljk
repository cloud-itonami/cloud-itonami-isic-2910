(ns automotive.operation
  "OperationActor -- one motor-vehicle-manufacturing operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Automotive Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Automotive Manufacturing
  ...
  One graph run = one motor-vehicle-manufacturing operation (intake ->
  ...
  decision to a human operator (the homologation engineer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/dispatch-vehicle`/`:actuation/issue-
  conformity-certificate` ALWAYS reach this node when the governor is
  clean -- see `automotive.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [automotive.automotiveadvisor :as automotiveadvisor]
...
(defn build
  ...
  opts:
    :advisor      -- an `automotive.automotiveadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (automotiveadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Automotive Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (automotiveadvisor/-advise advisor store request)]
            {:proposal p :audit [(automotiveadvisor/trace request p)]})))

      ;; Automotive Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
